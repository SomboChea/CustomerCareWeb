...
BEGIN
	...
	declare expect  date;
	declare day_between date;
	declare month_between date ;
	...
	ELSE 		
			...
			WHEN day_between<=7 THEN 7
			When day_between>7 and month_between=0 Then DATEDIFF(calldate, DATE_ADD(calldate ,INTERVAL 1 MONTH))
			end);
...
BEGIN
	...
	RETURN (Select name from tbl_name where id=@name_id);
END
;;
delimiter ;
