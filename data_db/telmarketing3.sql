...
BEGIN
	...
	Insert into tbl_address values(pid,did,cid,aid);
...
delimiter ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `Getlocname`(`ID` int) RETURNS varchar(50) CHARSET latin1
BEGIN
	#Routine body goes here...
	Declare locname varchar(255);
	set locname=(Select `name` from tbl_location where `id`=ID);
	RETURN locname;
END
;;
delimiter ;
